...
out_dir="/wes_ko_ukbb/data/saige/step1/cts"
dx mkdir -p ${out_dir}
for anc in eur; do
    ...
    # binary (all sex)
    cts_phenos="${pheno_dir}/cts_matrix_eur_header.txt"
    #bin_phenos="spiro_primary_malignancy_stomach"

    for pheno in $(cat $cts_phenos); do
        dx run saige-universal-step-1 \
                ...
                -i pheno_list=/wes_ko_ukbb/data/phenotypes/cts_matrix_eur.txt.gz \
                -i pheno="$pheno" \
